  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await axi.post(
        "/analytics/penalties/createFromExcel",
        formData,
        {
          headers: {
  ...
  return (
    <Card className="shadow-md rounded-2xl">
      <CardHeader>
        ...
        <CardDescription>
          Добавьте Excel-файл (.xlsx) с данными о штрафах
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form
  ...
  const fetchPenaltiesByYear = async () => {
    try {
      ...
      const [response2024, response2025] = await Promise.all([
        axi
          .get("/analytics/penalties/get?year=2024")
          .catch(() => ({ data: [] })),
        axi
          .get("/analytics/penalties/get?year=2025")
          .catch(() => ({ data: [] })),
      ]);
                    ...
                    <SelectContent>
                      ...
                      {Array.from({ length: 12 }, (_, i) => (
                        <SelectItem key={i + 1} value={(i + 1).toString()}>
                          {new Date(2000, i).toLocaleString("ru", {
                            month: "long",
                          })}
                        </SelectItem>
                      ))}
