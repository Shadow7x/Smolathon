...
export default function LeftSidePanel() {
  ...
  const menuItemClass = `flex items-center gap-1 ${
  ...
  const MenuItems = () => (
    <>
      ...
      <CardContent className={`${menuItemClass} group`}>
        <button
          className={`${
            isActive("/admin") ? "text-black font-semibold" : "text-[#636363]"
          } group-hover:text-black flex items-center gap-2 cursor-pointer`}
      ...
      <CardContent className={`${menuItemClass} group`}>
        <button
          className={`${
            isActive("/admin/EvacuationRoutes")
      ...
      <CardContent className={`${menuItemClass} group`}>
        <button
          className={`${
            isActive("/admin/TowTrucks")
              ? "text-black font-semibold"
              : "text-[#636363]"
          } group-hover:text-black flex items-center gap-2 cursor-pointer whitespace-normal text-left`}
      ...
      <CardContent className={`${menuItemClass} group`}>
        <button
          className={`${
            isActive("/admin/TrafficLightRegistry")
              ? "text-black font-semibold"
              : "text-[#636363]"
          } group-hover:text-black flex items-center gap-2 cursor-pointer`}
        ...
        >
          ...
          >
            <path
              ...
              fill={
                isActive("/admin/TrafficLightRegistry") ? "black" : "#636363"
              }
              className="group-hover:fill-black"
            />
          </svg>
      ...
      <CardContent className={`${menuItemClass} group`}>
        <button
          className={`${
            isActive("/admin/Downloadingreport")
              ? "text-black font-semibold"
              : "text-[#636363]"
          } group-hover:text-black flex items-center gap-2 cursor-pointer`}
        ...
        <button
          className={`${
            isActive("/admin/Map")
              ? "text-black font-semibold"
              : "text-[#636363]"
          } group-hover:text-black group-hover:font-semibold flex items-center gap-2 cursor-pointer`}
      ...
      {!isCollapsed && (
        <CardContent className="flex items-center gap-1">
          <div className="h-[2px] bg-[#636363] w-full"></div>
        </CardContent>
      )}

      {/* Футер */}
      <CardFooter
        className={`hidden md:flex items-center gap-1 cursor-pointer group font-semibold transition-all duration-300 ${
          isCollapsed ? "justify-center" : "justify-start"
        }`}
      ...
      >
        ...
        >
          <path
            d="M13.5417 8.66667V17.3333L17.875 13L13.5417 8.66667ZM5.41667 22.75C4.82083 22.75 4.31076 22.5378 3.88646 22.1135C3.46215 21.6892 3.25 21.1792 3.25 20.5833V5.41667C3.25 4.82083 3.46215 4.31076 3.88646 3.88646C4.31076 3.46215 4.82083 3.25 5.41667 3.25H20.5833C21.1792 3.25 21.6892 3.46215 22.1135 3.88646C22.5378 4.31076 22.75 4.82083 22.75 5.41667V20.5833C22.75 21.1792 22.5378 21.6892 22.1135 22.1135C21.6892 22.5378 21.1792 22.75 20.5833 22.75H5.41667ZM8.66667 20.5833V5.41667H5.41667V20.5833H8.66667ZM10.8333 20.5833H20.5833V5.41667H10.8333V20.5833Z"
            fill="#636363"
          />
        </svg>

        {!isCollapsed && (
          <span className="text-[#636363] group-hover:text-black">Скрыть</span>
        )}
      </CardFooter>
    </>
  );
  ...
  return (
    <>
      ...
      {/* Десктопная панель */}
      <div
        className={`ml-[2rem] transition-all hidden md:block ${
          isCollapsed ? "w-16" : "w-64"
        }`}
      >
        <Card className="sticky top-8">{MenuItems()}</Card>
      </div>
