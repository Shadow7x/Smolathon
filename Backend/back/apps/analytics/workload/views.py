        ...
        try:
            ...
            try:
                ...
                if list(trafficLight.columns) != [
                    ...
                    return Response(
                        "Некоректные данные", status=status.HTTP_400_BAD_REQUEST
                    )
                trafficLight.columns = ["detector", "date", "Car", "speed"]

                trafficLight = trafficLight.sort_values("Car").reset_index(drop=True)
                objs = []
                tek_car = Car()
                TIMES_INTERVAL = [
                    (f"{hour:02d}:00-{hour+2:02d}:00", f"{hour:02d}:00-{hour+2:02d}:00")
                ...
                for row in trafficLight.itertuples(index=False, name=None):
                    if row[2] != tek_car.name:
                        if objs != []:
                            ...
                            found_interval = next(
                                (
                                    i
                                    for i in TIMES_INTERVAL
                                    if time_interval_str >= i[0].split("-")[0]
                                    and time_interval_str < i[0].split("-")[1]
                                ),
                                None,
                            )
                    ...
                    objs.append(
                        Detection(
                            ...
                            time=(
                                timezone.make_aware(row[1])
                                if timezone.is_naive(row[1])
                                else row[1]
                            ),
                            car=tek_car,
                            speed=row[3],
    ...
    for i in range(len(detections)):
        if i + 1 < len(detections):
            workload = workloads.get(
                detections[i].detector.name + ":" + detections[i + 1].detector.name, {}
            )

        if i + 1 < len(detections):
            workload["count"] = workload.get("count", 0) + 1
            workload["start"] = [
                detections[i].detector.longitude,
                detections[i].detector.latitude,
            ]
            workload["end"] = [
                detections[i + 1].detector.longitude,
                detections[i + 1].detector.latitude,
            ]
            workloads[
                detections[i].detector.name + ":" + detections[i + 1].detector.name
            ] = workload
            ...
            workload["time"] = (
                workload.get("time", 0)
                + (detections[i + 1].time - detections[i].time).total_seconds() // 60
            )

    most_workloads = dict(
        (sorted(workloads.items(), key=lambda x: x[1]["count"], reverse=True))[:10]
    )
        ...
        if get.get("target"):
            ...
            if get.get("time_interval"):
                if target.workloads.filter(
                    ...
                    target_workloads = target.workloads.filter(
                        time_interval=get.get("time_interval")
                    )
                else:
                    return Response(
                        "Некоректный запрос", status=status.HTTP_400_BAD_REQUEST
                    )
            else:
                return Response(
                    "Некоректный запрос", status=status.HTTP_400_BAD_REQUEST
            ...
            for car in other_cars:
                if get.get("time_interval"):
                    other_workload = car.workloads.filter(
                        time_interval=get.get("time_interval")
                    )
                for workload in other_workload:
                    for detection in workload.detections.all():
                        ...
                        if (
                            ...
                            and detection.detector in target_nodes
                        ):
            ...
            print(count_other)
            count_other = list(
                filter(lambda x: x[1] >= nodes_count, count_other.items())
            )
            print(count_other)
